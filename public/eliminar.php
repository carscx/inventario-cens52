...
function rmdir_if_empty(string $dir): void
{
  ...
  while (($f = readdir($dh)) !== false) {
    ...
    if ($count > 0)
      break; // Si ya encontró uno, no necesita seguir.
  }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!csrf_check($token))
    $errors[] = 'Token CSRF inválido.'; // Valida el token.
  elseif (!$confirm)
    $errors[] = 'Debes confirmar la eliminación definitiva.'; // Valida la confirmación.
  else {
...
// Definir la variable para el Header
$header_title = 'Eliminar ítem';
...
<body>
  <main class="app-container">
    ...
    <article class="notice">
      ...
      <p>Ítem: <strong><?= h($item['nombre']) ?></strong> <span class="muted">[Código: <?= h($item['codigo']) ?>]</span>
      </p>
      <ul class="muted">
